...
const (
	// containerIDMuxName is the key used in gorilla/mux to extract the container ID
	// from the request URL for secret endpoints.
	containerIDMuxName = "containerIDMuxName"
...
// Uses AnythingRegEx to handle the case where containerID is empty (returns 400
// instead of 404).
var SplunkTokenPath = "/v2/splunk-token/" + utils.ConstructMuxVar(containerIDMuxName, utils.AnythingRegEx)

// ContainerEnvPath specifies the relative URI path for serving container
// environment variables.
var ContainerEnvPath = "/v2/container-env/" + utils.ConstructMuxVar(containerIDMuxName, utils.AnythingRegEx)
...
func SplunkTokenHandler(store secrets.SplunkTokenStore) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		containerID := mux.Vars(r)[containerIDMuxName]
		if containerID == "" {
			logger.Error("Empty container ID in splunk token request", logger.Fields{
				"requestType": requestTypeSplunkToken,
			})
			utils.WriteJSONResponse(w, http.StatusBadRequest, utils.ErrorMessage{
				Code:    "BadRequest",
				Message: "container ID is empty or malformed",
			}, requestTypeSplunkToken)
			return
		}

		token, ok := store.Get(containerID)
		if !ok {
			...
			utils.WriteJSONResponse(w, http.StatusNotFound, utils.ErrorMessage{
				Code:    "NotFound",
				Message: fmt.Sprintf("no secret found for container ID: %s", containerID),
			}, requestTypeSplunkToken)
			return
		}

		logger.Debug("Successfully served splunk token", logger.Fields{
			"containerID": containerID,
			"requestType": requestTypeSplunkToken,
		})
...
func ContainerEnvHandler(store secrets.ContainerEnvStore) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		containerID := mux.Vars(r)[containerIDMuxName]
		if containerID == "" {
			logger.Error("Empty container ID in container env request", logger.Fields{
		...
		logger.Debug("Successfully served container env", logger.Fields{
			"containerID": containerID,
			"requestType": requestTypeContainerEnv,
		})
